...
statenames = locationsdata.groupby('CVE_ENT')['NOM_ENT'].value_counts().reset_index().set_index('CVE_ENT')

UTC7 = ['Sinaloa','Sonora','Nayarit','Baja California','Baja California Sur']
UTC5 = ['Quintana Roo']
...
for j in range(1,367):
    ...
    localdata['statename'] = statenames['NOM_ENT'].loc[localdata['CVE_ENT']].values
    localdata['timezone'] = localdata['statename'].apply(GetTimeZone)
...
def LongitudeCorrection(long):
    return -4*long

def DayCorrection(J):
    return 450.8*np.sin((2*np.pi*J/365)-0.026903) + 595.4*np.sin((4*np.pi*J/365)+0.352835)
...
def ProcessDates(df):
    ...
    df['decease_date_comp'] = pd.to_datetime([str(val)+'-'+str(sal)+'-'+str(jal)+' '+str(pal)+':'+str(mal)  for val,sal,jal,pal,mal in zip(df['mes_ocurr'],df['dia_ocurr'],df['anio_ocur'],df['horas'],df['minutos'])],errors='coerce')
    
    df = df.dropna(subset=['decease_date'])
    df = df.dropna(subset=['decease_date_comp'])
...
def ProcessSolar(df,data=dtacont):
    ...
    df['timezone'] = data['timezone'].loc[dtatuple].values
    df['timezone_delta'] = [pd.DateOffset(hours=int(val[-1])) for val in df['timezone']]
    
    df['decease_date_UTC'] = pd.to_datetime(df['decease_date_comp'] + df['timezone_delta'])
    
    df['decease_date_solar'] = df['decease_date_comp'] - df['timezone_delta']
    df['longcorrection'] = df['long'].apply(LongitudeCorrection)
    df['daycorrection'] = df['dayofyear'].apply(DayCorrection)
    
    df['longcorrection'] = [pd.DateOffset(minutes=val) for val in df['longcorrection']]
    df['daycorrection'] = [pd.DateOffset(seconds=val) for val in df['daycorrection']]
    
    df['decease_date_solar'] = pd.to_datetime(df['decease_date_solar'] + df['longcorrection'] + df['daycorrection'])
    df['tod_solar'] = df['decease_date_solar'].dt.hour.astype(float) + df['decease_date_solar'].dt.minute.astype(float)/60
    
    df['fluxF'] = data['flux'].loc[dtatuple].values
    df['flux'] = [val(sal) for val,sal in zip(df['fluxF'],df['tod_solar'])]
...
dates0 = datacont['decease_date_UTC'].dt.to_pydatetime().tolist()
...
columns = ['decease_date', 'birth_date','decease_date_UTC', 'decease_date_solar',
           'decease_date_comp','tod','daylength', 'gdaylength', 'flux', 'gflux', 
           'lat', 'long','night','gr_lismex','desc','sexo','causa_def', 'Br', 
           'Bt', 'Bp', 'gBr', 'gBt', 'gBp']

datacont = datacont[columns]

datacont.to_csv('/media/tavo/storage/deathsmx/mxmortality_rev.csv',index=False)
